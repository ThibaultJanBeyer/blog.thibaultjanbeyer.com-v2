...
- 1.2. [Why is the Same Origin Policy useful?](#why-is-the-same-origin-policy-useful)
...
- 2.2. [What requests use CORS?](#what-requests-use-cors)
...
The Same-Origin Policy is a web-standart that prevents malicious external scripts to access sensitive data from one webpage. Generally it only permits a webpage to load data that comes from the same origin.
For example if a webpage on the origin `www.foo.com/bar` is requesting an image from `www.foo.com/image.png` it will work since the resource is on the same origin as the request comes from.
Now, when `www.foo.com/bar` wants to load an image from `www.baz.com/image.png` it is forbidden by the same origin policy since `baz.com` is not the same origin as `foo.com`. An origin is defined as a combination of [URI scheme](https://en.wikipedia.org/wiki/Uniform_Resource_Identifier#Examples), [domain](https://en.wikipedia.org/wiki/Hostname), and [port number](<https://en.wikipedia.org/wiki/Port_(computer_networking)>).  
So, before CORS came into play, it was almost impossible to load resources from other webpages. Now CORS is a mechanism that make it possible to obtain resources from other domains. We’ll see how in a minute.

### Why is the Same Origin Policy useful?

It’s a concept to isolate one website from another. It is what stops for example facebook accessing the data from your bank account.
<i class="hilite">It’s only the browser that enforces this rule. If it’s not a browser, then the same origin does not apply.</i>
...
In this mind example we are on the website hacker.com, this website opens a new browser tab for us which loads yourbank.com (suppose that’s the name of my bank). Now suppose I’m logged in already, hacker.com could read all information of that page. Or even worse, he could direct me to a login page and capture my credentials!  
...
This is a good example showing how one website/window/origin is isolated from another using the Same Origin Policy.
...
Here is some ancient example of frames. This is how websites used to be ~16 years ago. But frames are basically the same as `iframes` which you are probably more used to and same principles apply.
...
```JavaScript
// on www.hacker.com
fetch("https://hacker.com/info") // works
fetch("https://yourbank.com/transactions") //
```

Here the request is actually happening: the browser sends the request to your bank, the browser will receive the answer for your bank, but the browser will not let JavaScript access that response. You will get some kind of `No 'Access-Control-Allow-Origin' Headers present` or similar error message.

As you can see, the Same Origin Policy is a set of rules implemented by the browsers that protect one webpage from others in the same browser. It isolated pages from one another.

### So should we keep enforcing the Same Origin Policy?

It depends. While it might be a good layer of security. I can totally aggree with [ttwinlakkes](https://www.reddit.com/r/Frontend/comments/cfpf5a/what_are_your_painpoints_with_cors_cross_origin/euchfph/): most of the times I have had problems with it is with protected APIs that already have protections in place. It might be a good idea in general, but it is a nuisance if your only cross-origin resources are protected public endpoints.  
As you can see it really depends on your use case. Enabling Cross Origin Request for all domains **is not** a security risc per-se and can make total sense. For 99% of public APIs the Same Origin Policy makes absolutely no sense.

## Cross-Origin Resource Sharing (CORS) to the rescue!

With time came the conclusion that it is very often required to load resources from other origins. For example you might want to host your API endpoint on another origin than your webapp, i.e. `api.foo.com` vs `app.foo.com`.  
...
- Any `XMLHttpRequest` or `Fetch` request in JavaScript.
...
CORS allows a site to “opt-in” to weaken SOP and allow other sites to read data.

- Sender sends a request with an [origin header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Origin).
- Receiver answers with an [Access-Control-Allow-Origin header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Access-Control-Allow-Origin)
  the answer can be limited/opened even further with headers
...
There is an attribute for that! It’s called [crossorigin](https://developer.mozilla.org/en-US/docs/Web/HTML/CORS_settings_attributes) and here is how you use it:
...
- The Server still has to have CORS enabled and send the image with a `Access-Control-Allow-Credentials` Header
...
- Robwu has hoste a proxy for us: https://cors-anywhere.herokuapp.com/ | https://github.com/Rob--W/cors-anywhere/
...
Congratulations! You just learned all you have to know about CORS.

Key takeaways:

- The Same Origin Policy is only enforced by the browsers (frontend)
- CORS has to be enabled by on the server (backend)
- In the response, the `Access-Control-Allow-Origin` header has to be set.
- In the request the `Origin` header has to be set.

This, as [ttwinlakkes](https://www.reddit.com/r/Frontend/comments/cfpf5a/what_are_your_painpoints_with_cors_cross_origin/euchfph/) puts it nicely:

- It is a front-end dev’s problem that can only be fixed by a backend dev
- There’s (basically) no such thing as a CORS error. Instead, the server returns 405 Method Not Allowed because the server lacks an OPTIONS method handler.
- The main times I have received it are with protected APIs that already have protections in place. It might be a good idea in general, but it is mostly a nuisance if your only cross-origin resources are protected REST endpoints.

Since the Same Origin Policy is only enforced by the browser
